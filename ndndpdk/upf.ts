...
export function ndndpdkUP(ctx: NetDefComposeContext, upf: netdef.UPF): void {
  ...
  const n3cidr = new Netmask(ctx.c.networks.n3!.ipam.config[0]!.subnet).bitmask;
  ctx.finalize.push(() => setCommands(ctx, s, upf, n3cidr));
}

function setCommands(
    ctx: NetDefComposeContext,
    s: ComposeService,
    { name: ct, peers }: netdef.UPF,
    n3cidr: number,
): void {
  ...
  compose.setCommands(s, [
    ...
    `wait_ethdev() { test $(ndndpdk-ctrl list-ethdev 2>/dev/null | jq -s --arg MAC ${
    ...
    "ndndpdk-ctrl list-ethdev | tee ethdev.ndjson",
    ...
    `  ip addr add ${upfN3ip}/${n3cidr} dev $PASSTHRU_NETIF`,
